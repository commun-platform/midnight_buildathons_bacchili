...
BACCHIRI!━━Verifiable Measurement Layerは、疑似の1日分の測定値を1時間ごとの最小値・最大値にまとめ、値を隠したまましきい値判定を作ります。Wave 1の審査経路では、ユーザーが認可したブラウザクライアントを疑似計測元として使い、Midnight取引を認可します。公開記録は範囲内または範囲外で、測定値がない時間は範囲内とせず「停止」として区別します。
