...
物理センサーの真正性、連続測定、未提出データがないこと、デバイス側集計の正しさは証明しません。現行構成では、バックエンドと証明サーバーを信頼対象とします。ブラウザはPublic Midnight Indexerへ直接問い合わせて確定Contract Stateを照合しますが、Browser内でZK Verifierを再実行したりWitnessを開示したりはしません。
...
- Wave 2 — 計画: Local／複数Source照合の強化、署名付き来歴証明、運用自動化、障害復旧と複数デバイス監視。
- Wave 3 — 計画: 校正情報付きデバイス証明、ファームウェア識別情報、セキュアハードウェア連携、組織横断監査。

現行BrowserはPublic Midnight TX／Contract Stateを独立照合します。Browser内Proof Verifier実行、校正情報、Firmware識別情報の紐付けは現行機能ではなく計画です。
