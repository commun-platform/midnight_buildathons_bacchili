...
顧客価値を直接実現する回路です。コミットした非公開の時間別最小値・最大値から、対象デバイスへ登録したしきい値による運用日の各時間帯の公開判定を計算し、値を開示せずに証明します。`thresholdSatisfied`は日次総合結果として残します。

```mermaid
flowchart LR
    DSECRET["非公開: デバイス秘密値"] --> DAUTH["使用可能デバイスと権限を検査"]
    DAILY["非公開: 24時間枠<br/>+ 証明用乱数"] --> COMMIT["日次コミットメントを再計算"]
    PUBLIC["公開: デバイス、計測グループ、適用設定<br/>運用期間、時間枠の有無、24個の結果<br/>合計件数、日次総合結果、版番号"] --> BIND["公開・非公開情報を相互に固定"]
...
4. `measurementDay`内のAssignment登録済み`utcDayStartMinute`から始まり、正確に86,400秒。
