...
[TX hashによる第三者検証仕様](transaction_hash_verification.md)に分離しています。
...
flowchart LR
    ...
    POLICY --> DAILY
    ASSIGN --> DAILY
    DAILY --> RESULT["公開される24時間分の<br/>しきい値以内 / 範囲外 / 計測なし"]
```

管理用回路は運用管理者の秘密値を隠したまま、意図的に公開する設定を台帳へ書きます。日次回路は、
デバイス秘密値、時間別の最小値・最大値、時間別件数、証明用乱数を隠し、適用したしきい値、24個の時間帯別結果、日次の総合結果を公開します。
...
| 公開引数 | デバイス／しきい値／適用設定ID、UTC計測日／期間、時間枠の有無、24個の時間帯別結果、合計件数、日次総合結果、版番号 | 公開トランザクション入力 |
...
顧客価値を直接実現する回路です。コミットした非公開の時間別最小値・最大値から、対象デバイスへ登録したしきい値によるUTCの各時間帯の公開判定を計算し、値を開示せずに証明します。`thresholdSatisfied`は日次総合結果として残します。

```mermaid
flowchart LR
    DSECRET["非公開: デバイス秘密値"] --> DAUTH["使用可能デバイスと権限を検査"]
    DAILY["非公開: 24時間枠<br/>+ 証明用乱数"] --> COMMIT["日次コミットメントを再計算"]
    PUBLIC["公開: デバイス、計測グループ、適用設定<br/>UTC日・期間、時間枠の有無、24個の結果<br/>合計件数、日次総合結果、版番号"] --> BIND["公開・非公開情報を相互に固定"]
    ...
    RANGE --> RESULT["24個の公開判定と日次総合結果を<br/>再計算して一致を確認"]
...
トランザクションは、日次コミットメント、デバイスコミットメント、計測グループID、適用設定ID、UTC計測日と期間、
...
4. `measurementDay`が示すUTCの00:00:00から翌日00:00:00までと正確に一致。
...
11. 再計算した各時間帯の判定が、対応するPublic `hourResults`と一致する。
12. 観測済み各時間のANDが、日次のPublic `thresholdSatisfied`と一致する。

回路が`measurementDay`から期間境界を導出するため、UTC日次境界でない期間や部分日は失敗します。
...
hourResults[h] = 観測済みならしきい値以内または範囲外
hourResults[h] = 欠測なら計測なし
thresholdSatisfied = 観測済み全時間の判定をANDで結合 // 日次総合
```

停止時間はしきい値判定から除外します。全時間停止の日は計算上`thresholdSatisfied = true`ですが、回路が
再計算して公開する`observedHourCount = 0`を使い、アプリケーションでは「停止」と表示します。

時間帯別の次のすべてが正しい証明結果です。

- しきい値以内：その時間の非公開最小値・最大値が設定済みしきい値の範囲内
- 範囲外：その時間の非公開最小値または最大値が範囲外
- 計測なし：その時間がCanonical Absent Slot
...
グループ、デバイス、しきい値、適用設定、UTC計測日／期間、観測有無、24個の時間帯別結果、観測時間数／合計件数、版番号、日次総合結果、`verified: true`を含みます。時間別の最小値・最大値と証明用乱数は保存しません。
