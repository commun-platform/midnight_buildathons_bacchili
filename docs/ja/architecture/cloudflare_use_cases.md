...
この資料は審査用です。1枚に全経路を詰め込まず、同じ配置のままリソースを段階的に追加します。
...
着目点：第三者がTX hashを貼り付けると、運用日／登録済み境界、24個の時間帯別のしきい値以内／範囲外／計測なし、適用しきい値／有効期間、Device Commitment、Block、Midnight取引記録を確認できます。センサー値は見えません。
