...
Status: implementation-aligned specification, 2026-09-03 JST
...
The operational `sensor-registry` contract contains eight exported proof-generating circuits:
...
The first seven control who and what may participate. The eighth produces the daily private-threshold
...
experiments and are separated in [section 15](#15-development-only-circuit-profiles).

## 2. How the eight circuits fit together

```mermaid
flowchart LR
    OA["Private Operator secret"] --> ADMIN["7 administration proof circuits"]
...
| --- | --- | --- |
| Operator witness | `privateOperatorSecret()` | Private; used by seven administration circuits. |
...
| `rotateOperatorAuthority` | Knowledge of current Operator secret | Replace the Operator Authority with a higher version. |
...
| `registerPolicyAssignment` | Knowledge of Operator secret | Bind one policy and validity interval to one active Device/sensor stream without overlap. |
| `closePolicyAssignment` | Knowledge of Operator secret | Close the latest open Assignment at an explicit end time. |
...
It lets the current Operator rotate the private authority used by every administration circuit
without redeploying the contract. The replacement is public only as a one-way Authority hash.

| Part | Specification |
| --- | --- |
| Private input | Current `privateOperatorSecret()` |
| Public input | `newOperatorAuthority`, `newVersion` |
| Rejection rules | Empty or unchanged Authority, or a version that does not increase |
| Ledger update | Replace `operatorAuthority` and `operatorAuthorityVersion` atomically |

After confirmation, the old Operator secret no longer authorizes administration calls. The operator
tool writes the replacement secret to a mode-`0600` pending file before submission and activates it
only after the confirmed ledger state matches, so the rotation can be recovered after interruption.
...
| Rejection rules | Empty/reused Assignment ID, unknown Policy/Device, disabled Device, invalid interval, version `0`, an open predecessor, an overlap, or a non-increasing stream version |
| Ledger update | Store the Policy/Device/validity/version binding, update the latest Assignment for the Device/sensor stream, and increment `assignmentCount` |

`validUntil == 0` means no configured expiry. For each Device and public `sensorTypeCode`, only one
Assignment may remain open. A successor requires the previous Assignment to be closed, must start at
or after the previous end, and must use a larger version.

## 11. `closePolicyAssignment`

### What it achieves

It closes the latest open Assignment for one Device/sensor stream without modifying its Policy,
Device, operational-day boundary, start time, or version. Historical proofs remain valid.

| Part | Specification |
| --- | --- |
| Private input | `privateOperatorSecret()` |
| Public input | Assignment ID and explicit `validUntil` |
| Rejection rules | Unknown Assignment/Policy/stream, non-latest Assignment, already closed Assignment, or end not later than start |
| Ledger update | Replace only the Assignment's zero end time with the supplied `validUntil` |

A daily attestation may continue to use a closed Assignment only when its complete operational period
falls inside that Assignment's final validity interval.
...
### 12.3 Checks performed in one proof
...
## 13. Pure and internal helper circuits
...
| `derivePolicyAssignmentStreamKey` | Domain-separated Device/sensor stream key used to enforce Assignment order. | None; pure circuit. |
| `assertOperatorAuthorized` | Internal equality check used by seven administration circuits. | Included inside the calling proof. |
...
## 15. Development-only circuit profiles
...
## 16. Source, artifacts, and validation
...
- eight ZKIR/BZKIR pairs: `midnight/contracts/sensor-registry/src/managed/sensor-registry/zkir/`;
- eight prover/verifier key pairs: `midnight/contracts/sensor-registry/src/managed/sensor-registry/keys/`; and
