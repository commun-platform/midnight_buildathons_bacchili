...
| --- | --- | --- |
| **Daily Proof record list / Open** | `GET /api/v1/public/proofs` returns newest-first confirmed, redacted records. Opening a date calls `GET /api/v1/public/proofs/:proofJobId`. No Wallet is required. | The selected record shows Policy, assignment, public result, commitment, observed/STOPPED counts, transaction evidence, and the actual ZKP generation timestamp. |
