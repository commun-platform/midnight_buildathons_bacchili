...
| **Refresh Policies** | Explicitly fetches Policy status and updates only affected components, preserving unfinished forms and focus. The button is disabled during the request and enabled again on completion or failure. | Reads the current Project Policy operations and refreshes its authenticated configuration when the registered Policy list changes. | Policy status does not poll automatically. Reload again while registration is pending; server processing continues independently. |
