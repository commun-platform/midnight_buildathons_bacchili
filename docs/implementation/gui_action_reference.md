...
| **+ New Policy** | Opens name, mode, and bound fields. Modes are closed range, upper bound only, and lower bound only. Temperature inputs use 0.01 °C steps. **Register Policy** gets a one-time challenge and asks the Browser Wallet to sign the exact Project, Policy ID, mode, centi-degree bounds, nonce, and timestamp; **Cancel** makes no change. | The Worker verifies Project ownership, freshness, one-time nonce, canonical values, signature, and quota. It writes a stable Policy operation and enqueues its ID. The Server Wallet Container uses its separate private Operator Authority to call `registerThresholdPolicy`; after Indexer confirmation, D1 stores the public Policy mirror for that Project. | Maximum 10 registered plus pending Policies per Project. A registered Policy is immutable; a different threshold is a new Policy. Queue/retry progress survives browser closure and is restored by operation ID. While a Policy is processing, the row shows an approximate start time for the active `always-on`, `on-demand`, or `scheduled` profile. Bounds are public on Midnight. |
...
| **Refresh** | Explicitly fetches Policy status and updates only affected components, preserving unfinished forms and focus. The compact button is disabled during the request and enabled again after completion, failure, or timeout. | Reads the current Project Policy operations and refreshes its authenticated configuration when the registered Policy list changes. | Policy status does not poll automatically. Refresh again while registration is pending; server processing continues independently. |
...
public Proof records. Explicit Policy reload keeps a registered or in-flight row visible until the refreshed
Project configuration contains it. Component status refreshes preserve unfinished Project and Policy form
...
| **Refresh state beside a Device workflow action** | Re-reads the selected Device registration, generated day, Proof/TX, or daily-history state without reloading the whole page. The control stays beside the action that is waiting for asynchronous server confirmation and preserves the current form and selection. | Reads the authoritative Device-scoped status from Worker/D1 and resumes the pending registration or Proof Job by its stable ID. | The affected action becomes enabled as soon as the server-confirmed state is available. |
