...
readonly DEFAULT_OUTPUT_DIR="${REPO_ROOT}/.device-release/archives"
...
usage() {
  cat <<'EOF'
Usage: ./edge-device/release/package_archive.sh [options]
...
resolve_defaults() {
  if [[ -z "${FIRMWARE_VERSION}" ]]; then
    FIRMWARE_VERSION="$(node -e "const p=require(process.argv[1]); process.stdout.write(p.version)" "${REPO_ROOT}/package.json")"
  fi
...
require_development_host() {
  node "${REPO_ROOT}/tools/repository-checks/require-development-host.mjs"
...
main() {
  ...
  require_development_host
  resolve_defaults
  node "${REPO_ROOT}/tools/repository-checks/verify-repository-portability.mjs"
