...
const artifactsRoot = path.join(repoRoot, 'contracts', 'daily-attestation', 'src', 'managed');
...
function toolchainMetadata() {
  const developmentPackage = JSON.parse(
    fs.readFileSync(path.join(repoRoot, 'apps', 'development', 'operator-cli', 'package.json'), 'utf8'),
  );
  return {
